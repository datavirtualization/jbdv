...
/**
 * Used to discover the ModeShape repository sequencer components.
 */
public class SequencerDiscoveryComponent implements ResourceDiscoveryComponent<SequencerComponent> {
     ...
     */
    @Override
    public Set<DiscoveredResourceDetails> discoverResources( final ResourceDiscoveryContext<SequencerComponent> context )
        throws Exception {
        ...
        final Result result = connection.execute(Operation.Util.createRhqOperation(RepositoryComponent.createGetSequencers(),
                                                                                   repoAddr));
        ...
        if (result.isSuccess()) {
            ...
            if ((temp != null) && (temp instanceof Map<?, ?>)) {
                final Map<?, ?> seqMap = (Map<?, ?>)temp;

                if (seqMap.isEmpty()) {
                    ModeShapePlugin.LOG.debug("No sequencers discovered in repository '" + repository.deploymentName() + '\'');
                } else {
                    ...
                    for (final Object key : seqMap.keySet()) {
                        ...
                        final String seqName = (String)key;

                        detail = new DiscoveredResourceDetails(context.getResourceType(), seqName, seqName,
                                                               ModeShapePlugin.VERSION,
                                                               context.getResourceType().getDescription(), config, null);

                        discoveredResources.add(detail);
                        ModeShapePlugin.LOG.debug("Discovered ModeShape Sequencer '" + seqName + "' in repository '"
