...
/**
 * Used to discover the ModeShape repository authenticator components.
 */
public class AuthenticatorDiscoveryComponent implements ResourceDiscoveryComponent<AuthenticatorComponent> {
     ...
     */
    @Override
    public Set<DiscoveredResourceDetails> discoverResources( final ResourceDiscoveryContext<AuthenticatorComponent> context )
        throws Exception {
        ...
        final Result result = connection.execute(Operation.Util.createRhqOperation(RepositoryComponent.createGetAuthenticators(),
                                                                                   repoAddr));
        ...
        if (result.isSuccess()) {
            ...
            if ((temp != null) && (temp instanceof Map<?, ?>)) {
                final Map<?, ?> authenticatorMap = (Map<?, ?>)temp;

                if (authenticatorMap.isEmpty()) {
                    ModeShapePlugin.LOG.debug("No authenticators discovered in repository '" + repository.deploymentName() + '\'');
                } else {
                    final Set<DiscoveredResourceDetails> discoveredResources = new HashSet<DiscoveredResourceDetails>(
                                                                                                                      authenticatorMap.size());
                    DiscoveredResourceDetails detail = null;

                    for (final Object key : authenticatorMap.keySet()) {
                        ...
                        final String authenticatorName = (String)key;

                        detail = new DiscoveredResourceDetails(context.getResourceType(), authenticatorName, authenticatorName,
                                                               ModeShapePlugin.VERSION,
                                                               context.getResourceType().getDescription(), config, null);

                        discoveredResources.add(detail);
                        ModeShapePlugin.LOG.debug("Discovered ModeShape Authenticator '" + authenticatorName
